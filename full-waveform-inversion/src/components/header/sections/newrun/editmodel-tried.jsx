...
export default function EditModelData({ modelType, selectedItem, handleSelectedFiles }) {
    const [isOpen, setIsOpen] = useState(false);
    // const [formValues, setFormValues] = useState(inputValues);  // Local form state
    const [formValues, setFormValues] = useState(selectedItem.jsonData);  // Local form state
    ...
    const handleSave = () => {
        handleSelectedFiles(prev => ({
            ...prev, // Correctly spread the previous state
            [modelType]: {
                ...prev[modelType], // Spread the existing properties of the modelType
                jsonData: formValues // Update jsondata with formValues
            }
        }));
