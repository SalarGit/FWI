...
export default function EditModelData({ handleModel, selectedModel, modelType }) {
    ...
    useEffect(() => {
        if (isOpen) {
            console.log("dropdown opened, resetting formValues to selectedModel.jsonData");
            setFormValues(selectedModel.jsonData);  // Reset form values when dropdown is opened
        }
